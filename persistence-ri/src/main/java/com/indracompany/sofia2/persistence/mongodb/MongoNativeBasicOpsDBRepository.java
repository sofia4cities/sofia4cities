...
@Lazy
@Slf4j
public class MongoNativeBasicOpsDBRepository implements BasicOpsDBRepository {
	@Autowired
	...
	@Override
	public List<String> remove(String collection, String query) throws DBPersistenceException {
	...
	public List<String> update(String collection, String query, String data) throws DBPersistenceException {
		try {
			...
			for (BasicDBObject document : cursor) {
				/*
				 * En 'datos' llega sólo la instancia de la ontología, sin
				 * ContextData. En el documento a actualizar debemos poner al
				 * mismo nivel el ContextData y la instancia de la ontología. El
				 * bucle hace falta porque no siempre la instancia de ontología
				 * tiene una única clave de primer nivel y el resto de campos
				 * anidados bajo ella.
				 */
	...
	@Override
	public List<String> update(String collection, String statement) throws DBPersistenceException {
		...
		try {
			 ...
			 * specify an update operator. We should remove this inconsistency.
			 */
			String updateOperation = util.prepareQuotes(data);
			String result = getUpdateStatement(data, collection);
			return update(collection, query, updateOperation);
		} catch (Exception e) {
	...
	@Override
	public String findAsOneJSON(String collection, String query) throws DBPersistenceException {
		return findAsOneJSON(collection,query, numMaxRegisters);
	}
	@Override
	public String findAsOneJSON(String collection, String query, int limit) throws DBPersistenceException {
		log.debug("find", query, collection);
		try {
			query = util.convertObjectIdV1toVLegacy(query);
			return JSON.serialize(find(collection,query,limit));
		} 
		catch (javax.persistence.PersistenceException e) {
			log.error("find", e, query, collection);
			throw new PersistenceException(e);
		} 
	}

	@Override
	public List<String> find(String collection, String query) throws DBPersistenceException {
		return find(collection,query, numMaxRegisters);
	}
	@Override
	...
	@Override
	public String findAllAsOneJSON(String collection) throws DBPersistenceException {
		return JSON.serialize(findAll(collection));
	}

	@Override
	public String findAllAsOneJSON(String collection, int limit) throws DBPersistenceException {
		return JSON.serialize(findAll(collection));
	}

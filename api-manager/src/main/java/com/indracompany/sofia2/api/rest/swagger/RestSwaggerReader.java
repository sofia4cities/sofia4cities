...
public class RestSwaggerReader {
	...
	static {
		...
		sofia2Header.setName(XSOFIA2APIKey);
		sofia2Header.setRequired(false);
		sofia2Header.ty
		 serializableParameter.setItems(new StringProperty());
		
		 {
             "name": "X-SOFIA2-APIKey",
             "in": "header",
             "required": false,
             "type": "string"
         }*/
    ...
    public Swagger read(ApiDTO apiDto, BeanConfig config) {
		...
		Tag tag = new Tag();
		tag.setName(apiDto.getIdentificacion());
		tags.add(tag);
	...
	/*	Map<String, Model> definitions=new HashMap<String, Model>();
		...
		for (String type : types) {
            ...
            RestSwaggerReaderHelper.appendModels(clazz, swagger);
        }
		...
		*/
        ...
        ArrayList<OperacionDTO> operations = apiDto.getOperaciones();
    ...
    private ApiDTO populateApiDTOLite(ApiDTO apidto)
    {
    	...
    	api2.setOperaciones(null);
    	api2.setAutenticacion(null);
    	return api2;
    }
    ...
    private void parse(Swagger swagger, OperacionDTO operacionDTO) {
    	...
    	ArrayList<ApiQueryParameterDTO> queryParams = operacionDTO.getQueryParams();
         ...
         Parameter sofia2Api = RestSwaggerReaderHelper.populateParameter(swagger, "X-SOFIA2-APIKey", "X-SOFIA2-APIKey", false, "header", "string", null,null);
         ...
         for (ApiQueryParameterDTO apiQueryParameterDTO : queryParams) {
        	 ...
        	 Parameter parameter = RestSwaggerReaderHelper.populateParameter(swagger, name, description, false, condition, type, null,value);

<html>
 ...
 <div style="display: none;" id="dialog-error" title="Se ha producido un error">Se ha perdido la comunicación con la plataforma</div>    
	...
	<script type="text/javascript">
		...
		RED.nodes.registerType('sofia2-query-static',{
			category: 'function',
			color: '#C0DEED',
			defaults: {
				name: {value:""},
			...
			icon: "db.png",
			label: function() {
				return this.name||"sofia2-query-static";
			}
		});
	...
	function showErrorDialogStatic() {
     ...
     var urlService = sofia2Config.scriptBasePath + '/sofia2/flowengine/node/services/user/ontologies?authentication=';
	...
	<script type="text/x-red" data-template-name="sofia2-query-static">
		...
		<div class="form-row">
			<label for="node-input-targetDB"><i class=" icon-share-alt"></i> Database</label>
			<select id="node-input-targetDB" placeholder="Database">
				<option value="input"> BDTR</option>
				<option value="output"> BDH</option>
			</select>
		</div>
		<div class="form-row">
...
      	
	...
	<script type="text/x-red" data-help-name="sofia2-query-static">
	    ...
	    <p><code>Base de datos</code> Selection of different data bases.</p>

...
public class MongoStatementParser implements DBStatementParser {
	...
	public static TableResult getTableNamesFromMongoStatement(String mongoStmt) throws NotSupportedStatementException {
		String operation;
		final TableResult result=new TableResult();
		if(mongoStmt.toLowerCase().contains(SELECT_MONGO_LITERAL)){
			...
			throw new NotSupportedStatementException("The statement: "+mongoStmt+" can not be processed to get ontologies acceded");
		}

		final String colName=MongoQueryNativeUtil.getCollNameFromAction(mongoStmt, operation);

...
public class ApiManagerService {
	...
	public Api getApi(String pathInfo, User user) {
		...
		/*String apitipo=null;
		if (pathInfo.startsWith(openDataPath)) {
			pathInfo = pathInfo.substring(openDataPath.length());
			apitipo="OData";
		} else if (pathInfo.startsWith(webServicePath)) {
			pathInfo = pathInfo.substring(webServicePath.length());
			apitipo = WEB_SERVICE_API;
		}
		*/
		...
		//Comprobamos si está disponible --> publicada, deprecada, en desarrollo o creada y el usuario es propietario
	...
	public String getApiVersion(String pathInfo) throws BadRequestException{
		...
		//Comprueba que existe el delimitador a partir del que vendrá el identificador de la ontologia
		if (slashIndex==-1) {
	...
	public Api getApi(String apiIdentifier, int apiVersion, String tipoapi)throws BadRequestException, ForbiddenException {
		Locale locale = LocaleContextHolder.getLocale();
	...
	public ApiOperation getCustomSQL(String pathInfo, Api api){
		...
		// Se recorren las operaciones de la API, buscando las que coincidan por metodo HTTP y por Path
	...
	public HashMap<String, String> getCustomParametersValues(HttpServletRequest request, HashSet<ApiQueryParameter> queryParametersCustomQuery) {
		Locale locale = LocaleContextHolder.getLocale();
		...
		for (ApiQueryParameter customqueryparameter : queryParametersCustomQuery) {
			...
			if (paramvalue == null){
				// No se encuentra el valor del parametro configurado en la operacion en la peticion
				...
				if (customqueryparameter.getTipo().equals(Constants.API_TIPO_DATE)){
					try {
				...
				} else if (customqueryparameter.getTipo().equals(Constants.API_TIPO_STRING)){
					try {
						paramvalue.toString();
						paramvalue = "'" + paramvalue + "'";
					} catch (Exception e) {
				...
				} else if (customqueryparameter.getTipo().equals(Constants.API_TIPO_NUMBER)){
					try {
				...
				} else if (customqueryparameter.getTipo().equals(Constants.API_TIPO_BOOLEAN)){
	...
	/*public void checkApiLimit(Api api){
		Locale locale = LocaleContextHolder.getLocale();
		if(cacheApiLimitService.isLimit(api)){
			Integer timeLeft=cacheApiLimitService.getApiLimitTimeoutLeft(api.getId());
			LogService.getLogI18n(this.getClass()).debug("ApiService.debug.peticiones", api.getEndpoint(), cacheApiLimitService.getApiRequestsNumber(api.getId()));

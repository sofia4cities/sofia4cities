    ...
    function processDataToGadget(data){ //With dynamic loading this will change
      switch(vm.config.type){
        ...
        case "pie":
          ...
          var allLabelsField = [];
          for(var index=0; index < vm.measures.length; index++){
            allLabelsField = allLabelsField.concat(data.map(function(d){return getJsonValueByJsonPath(d,vm.measures[index].config.fields[0])}));
          }
          ...
          for(var index=0; index < vm.measures.length; index++){
            var dataRawSerie = data.map(function(d){return getJsonValueByJsonPath(d,vm.measures[index].config.fields[1])});
            var labelRawSerie = data.map(function(d){return getJsonValueByJsonPath(d,vm.measures[index].config.fields[0])});
            var sortedArray = [];
        ...
        case "map":
          ...
          vm.markers = data.map(
            function(d){
              return {
                lat: getJsonValueByJsonPath(d,vm.measures[0].config.fields[0]),
                lng: getJsonValueByJsonPath(d,vm.measures[0].config.fields[1]),

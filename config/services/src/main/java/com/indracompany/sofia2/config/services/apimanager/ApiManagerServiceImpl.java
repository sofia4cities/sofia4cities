...
public class ApiManagerServiceImpl implements ApiManagerService {
	...
	public List<Api> loadAPISByFilter(String apiId, String state, String userId, String loggeduser) {
		...
		if ((apiId == null || "".equals(apiId)) && (state == null || "".equals(state))
	...
	private List<Api> filterApisByUserAndState(List<Api> apis, User user) {
		...
		for (Api api : apis) {
			if ((user.getRole().getId().equals(Role.Type.ROLE_ADMINISTRATOR.name())) || (api.getUser().equals(user))
					|| (!(api.getState().equals(Api.ApiStates.CREATED)
							|| api.getState().equals(Api.ApiStates.DELETED)))) {
	...
	public String createApi(Api api, String operationsObject, String authenticationObject) {
		...
		String numversionData = "{\"identification\":\"" + api.getIdentification() + "\",\"apiType\":\""
				+ api.getApiType() + "\"}";
	...
	public void removeAuthorizationByApiAndUser(String apiId, String userId) {
		UserApi userApi = this.userApiRepository.findByApiIdAndUser(apiId, userId);
		if (userApi != null) {
			userApiRepository.delete(userApi);
		}
